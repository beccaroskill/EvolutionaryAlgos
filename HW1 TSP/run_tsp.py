...
def run_ga_kmeans_hybrid_search(points, n_trials, n_processes, r_mutate=0.8, inverted=False):
    return run_ga_search(points, n_trials, n_processes, True, inverted, r_mutate, 
                  True, True, recombination_types=['intra-region'], 
                  mutation_types=['swap intra-region'])

def run_rmhc_kmeans_hybrid_search(points, n_trials, inverted=False):
    return run_rmhc_search(points, n_trials, order_by_cluster=True,
                           initialize_by_cluster=True, plot=True, inverted=inverted)
...
if __name__ == '__main__':
    ...
    #     df.to_csv('results/tsp1/longest/random_n{}_i{}.csv'.format(n_trials, i))
    
    # RMHC
    # for i in range(1,2):
    #     (df, path) = run_rmhc_search(points, n_trials,inverted=longest_path)
    #     df.to_csv('results/tsp1/longest/rmhc_n{}_i{}.csv'.format(n_trials, i))
    
    # # GA
    # for i in range(1,6):
    #     (df, path) = run_ga_search(points, n_trials, 50, inverted=longest_path)
    #     df.to_csv('results/tsp1/longest/ga_n{}_i{}.csv'.format(n_trials, i))
    
    longest_path = False
    # Random
    # for i in range(1,2):
    #     (df, path) = run_random_search(points, n_trials,inverted=longest_path)
    #     df.to_csv('results/tsp1/shortest/random_n{}_i{}.csv'.format(n_trials, i))
    
    # # # RMHC
    # # for i in range(1,2):
    # #     (df, path) = run_rmhc_search(points, n_trials,inverted=longest_path)
    # #     df.to_csv('results/tsp1/shortest/rmhc_n{}_i{}.csv'.format(n_trials, i))
    
    # GA
    for i in range(1,6):
        (df, path) = run_rmhc_kmeans_hybrid_search(points, n_trials)
        df.to_csv('results/hybrid_rmhc_n{}_i{}.csv'.format(n_trials, i))
    ...
    # # for i in range(1,2):
    # #     (df, path) = run_random_search(points, n_trials,inverted=longest_path)
    # #     df.to_csv('results/tsp2/longest/random_n{}_i{}.csv'.format(n_trials, i))
    
    # # # RMHC
    # # for i in range(1,2):
    # #     (df, path) = run_rmhc_search(points, n_trials,inverted=longest_path)
    # #     df.to_csv('results/tsp2/longest/rmhc_n{}_i{}.csv'.format(n_trials, i))
    
    # # GA
    # for i in range(1,6):
    #     (df, path) = run_ga_search(points, n_trials, 50, inverted=longest_path)
    #     df.to_csv('results/tsp2/longest/ga_n{}_i{}.csv'.format(n_trials, i))
    
    # longest_path = False
    # # Random
    # # for i in range(1,2):
    # #     (df, path) = run_random_search(points, n_trials,inverted=longest_path)
    # #     df.to_csv('results/tsp2/shortest/random_n{}_i{}.csv'.format(n_trials, i))
    
    # # # RMHC
    # # for i in range(1,2):
    # #     (df, path) = run_rmhc_search(points, n_trials,inverted=longest_path)
    # #     df.to_csv('results/tsp2/shortest/rmhc_n{}_i{}.csv'.format(n_trials, i))
    
    # # GA
    # for i in range(1,6):
    #     (df, path) = run_ga_search(points, n_trials, 50, inverted=longest_path)
    #     df.to_csv('results/tsp2/shortest/ga_n{}_i{}.csv'.format(n_trials, i))
